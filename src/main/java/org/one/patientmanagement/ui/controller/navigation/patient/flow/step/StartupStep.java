 ...
 * @author KAROL JOHN
 */
public class StartupStep implements FlowStep<PatientRoute, PatientViewModel> {

    @Override
    public PatientRoute getRoute() {
        return PatientRoute.STARTUP;
    }

    @Override
    public FlowStep<PatientRoute, PatientViewModel> next(FlowState<PatientViewModel> state) {
        switch(state.data().getPatientType()) {
            case EXISTING -> {
            ...
            default -> throw new AssertionError(state.data().getPatientType().name());
        }
    }
